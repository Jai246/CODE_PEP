public class pepYtListSearchingSorting 
{
  ...
  static class Pair {
    ...
    // Solved using Binary Search and backtracking
    // we have to use backtracking because , in other questions Agressive cows , Allocate Minimum no of pages
    // the order of palcement has to be maintained acc to the order of array but in this question it is not
    // necessary 
    // for eg [1 2 3 4 5 6]
    // In questions like Agressive cows , Allocate Minimum mandatorily subarray has to be maintained ie we can't 
    // skip elements but here in this question we can skip elements ie we can take 1->4->6

    public boolean check(int[]a , int k , int val ,int sum, boolean[]vis , int n)
    {
        if(k >= 1 && n == a.length) return true;   
        ...
        for(int i = 0;i<a.length;i++)
        {
           ...
           {
                vis[i] = true;
                if(a[i] > val) res =  false;
                else if(sum+a[i] <= val) res = res || check(a,k,val,sum+a[i],vis,n+1);
                else res = res || check(a,k-1,val,a[i],vis,n+1);
                vis[i] = false;
           }
        }
        return res;
    ...
    public int distributeCookies(int[] cookies, int k) 
    {
        ...
        {
            int mid = (i+j)/2;
            boolean[] vis=new boolean[cookies.length];
            boolean res = check(cookies,k,mid,0,vis,0);
            if(res) j = mid; 
            else i = mid+1; 
        }
        return j;
